...
ap.add_argument("--rounds", default="1,5,13")
ap.add_argument("--alphas", default="inf")
...
ap.add_argument("--out", default=f"{NFS}/iclscan-decentralized/results/noniid/"
                                 "alignins/alignins_freerider_control.json")
...
@torch.no_grad()
def stats_for_group(fac, ids, ref_id):
...
t0 = time.time()
for al in ALPHAS:
    for scen in ("att", "freerider"):
        run = ("r15_alphainf_freerider_seed0" if scen == "freerider"
               else f"r25_alpha{al}_{scen}_seed0")
        for r in ROUNDS:
            for rcv in range(8):
                ...
                st = stats_for_group(fac, ids, ref_id=rcv)
    ...
    for al in ALPHAS:
        for r in ROUNDS:
            ...
            for rcv in NBR_OF_ATT:
                c = res["cells"].get(f"{al}|att|r{r}|recv{rcv}")
                if not c:
                    continue
                att.append(c["0"][stat])
            ...
            cn = []
            for rcv in NBR_OF_ATT:
                c = res["cells"].get(f"{al}|freerider|r{r}|recv{rcv}")
                if c:
                    cn += [c[str(i)][stat] for i in ADJ[rcv]]
            if not att:
                continue
            a = float(np.mean(att))
            b = float(np.mean(ben)) if ben else float("nan")
            worst = max(abs(x) for x in ben) if ben else float("nan")
            sep = "SEPARATES" if abs(a) > worst else "overlaps"
            print(f"    alpha={al:>4} r{r:<3} attacker {a:+7.2f} | benign(att-run) {b:+7.2f} "
                  f"worst |z| {worst:5.2f} | clean-run {np.mean(cn) if cn else float('nan'):+7.2f}"
                  f"   {sep}")
    print()
